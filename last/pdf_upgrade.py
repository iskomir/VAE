...
pdf_min, pdf_max = np.quantile(pdf_values, [0.05, 0.95])
pdf_pad = (pdf_max - pdf_min) * 0.1

plt.hist(pdf_values[filtered_metadata == 1], 
         bins=np.linspace(pdf_min-pdf_pad, pdf_max+pdf_pad, 50),
...
plt.xlim(pdf_min-pdf_pad, pdf_max+pdf_pad)
...
pad = 0.1 * max(x_max-x_min, y_max-y_min)  # 10% padding

# Создаем маски для разных групп
normal_mask = (filtered_metadata == 1)
anomaly_mask = (filtered_metadata == 0)
top_anomaly_mask = np.zeros_like(filtered_metadata, dtype=bool)
top_anomaly_mask[top_100_indices] = True

# 3. Латентное пространство (первые 2 измерения)
plt.subplot(1, 3, 3)

scatter = plt.scatter(latent_representations[:, 0], latent_representations[:, 1], 
                     c=np.where(filtered_metadata == 1, 'b', 'r'), 
                     alpha=0.3, s=5)

# Топ-аномалии 
top_scatter = plt.scatter(latent_representations[top_100_indices, 0], 
...
legend_elements = [
    Line2D([0], [0], marker='o', color='w', label='Normal',
          markerfacecolor='b', markersize=8, alpha=0.7),
    Line2D([0], [0], marker='o', color='w', label='Anomalies',
          markerfacecolor='r', markersize=8, alpha=0.7),
    Line2D([0], [0], marker='o', color='w', label='Top Anomalies',
          markerfacecolor='yellow', markeredgecolor='k', markersize=12)
]
...
plt.tight_layout()
plt.savefig('vae_pdf_results_16.pdf', format='pdf', bbox_inches='tight')