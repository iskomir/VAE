...
# VAE с архитектурой как в первом коде
...
# Разбиваем данные на обучающую и валидационную выборки (только нормальные данные)
normal_data = filtered_data[filtered_metadata == 1]
train_data, val_data = train_test_split(normal_data, test_size=0.3, random_state=seed)
...
# Получение скрытых представлений для всех данных
...
# Параметры многомерного нормального распределения (только на нормальных данных)
# normal_latent = latent_representations[filtered_metadata == 1]  # Обучение только на нормальных данных
...
# Сортировка по возрастанию плотности и выбор 100 самых аномальных
sorted_indices = np.argsort(pdf_values)  # Сортируем по возрастанию плотности
...
# Построение гистограммы для плотности распределения
plt.figure(figsize=(8, 4))
plt.hist(pdf_values, bins=30, density=True, alpha=0.6, color='g')
plt.title('Гистограмма PDF')
plt.xlabel('pdf_values')
plt.ylabel('Плотность')
plt.grid(True)
plt.savefig('pdf_values.pdf', format='pdf', bbox_inches='tight')
plt.close()